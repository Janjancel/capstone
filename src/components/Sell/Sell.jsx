...
const Sell = () => {
  ...
  const onDrop = (e, type) => {
    ...
    if (!file || !file.type.startsWith("image/")) {
      ...
      toast.error("Please drop a valid image file.", { id: TOAST_ID });
      return;
    }
    handleImageChange(file, type);
  };
  ...
  const geocodeAddress = async () => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        location: { lat: +data[0].lat, lng: +data[0].lon },
      }));

      toast.dismiss(TOAST_ID);
      toast.success("Address pinned on map.", { id: TOAST_ID });
    } catch {
      toast.dismiss(TOAST_ID);
      toast.error("Address not found.", { id: TOAST_ID });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userId) {
      toast.dismiss(TOAST_ID);
      toast.error("Please login to submit.", { id: TOAST_ID });
      return;
    }
    ...
    try {
      ...
      ["frontImage", "sideImage", "backImage"].forEach(
        (k) => formData[k] && payload.append(k, formData[k])
      );
      ...
      ["frontImage", "sideImage", "backImage"].forEach(resetPicker);
    } catch (err) {
      toast.error("Submission failed. Please try again.", { id: TOAST_ID });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const renderDropzone = (type, label) => (
    <Box mt={2}>
      <input
        ...
        accept="image/*"
        hidden
        onChange={(e) => handleImageChange(e.target.files?.[0], type)}
      />
      ...
      >
        {previewUrls[type] ? (
          <Box
            component="img"
            src={previewUrls[type]}
            sx={{ width: "100%", maxHeight: 140, objectFit: "cover" }}
          />
        ) : (
          <Typography variant="caption" color="text.secondary">
              ...
              <form onSubmit={handleSubmit}>
                <Box display="flex" gap={2} mb={3}>
                  <TextField
                    ...
                    onChange={(e) => setSearchAddress(e.target.value)}
                  />
                  <Button onClick={geocodeAddress} variant="contained">
                    Search
                  </Button>
                </Box>
                ...
                <TextField fullWidth label="Item Name" name="name" required margin="normal" value={formData.name} onChange={handleChange} />
                <TextField fullWidth label="Contact No" name="contact" required margin="normal" value={formData.contact} onChange={handleChange} />
                <TextField fullWidth label="Price" type="number" name="price" required margin="normal" value={formData.price} onChange={handleChange} />
                <TextField fullWidth multiline rows={3} label="Description" name="description" required margin="normal" value={formData.description} onChange={handleChange} />
