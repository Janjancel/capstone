...
// Use axios instance so baseURL is consistent and easy to manage
...
export default function LoginForm({ onSuccess, toggleMode }) {
  ...
  const formik = useFormik({
    // rename field to 'identifier' to match server payload and avoid confusion
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      ...
      try {
        ...
        if (status === 401 || (msg && msg.toLowerCase().includes("invalid"))) {
          toast.error(msg || "Invalid credentials. Please check your email/username and password.");
        ...
        setSubmitting(false);
        setLoading(false);
        // don't clear the fields on every failure — keep values so user can retry
      }
    },
  });
