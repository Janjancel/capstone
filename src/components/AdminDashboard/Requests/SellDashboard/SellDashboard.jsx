...
const SellDashboard = () => {
  ...
  const TERMINAL_STATUSES = ["accepted", "accept", "declined", "decline", "cancelled", "canceled", "cancel"];

  const normalizeHistory = (req) => {
    const raw =
      req?.statusHistory ||
      req?.history ||
      req?.statusTimeline ||
    ...
    if (Array.isArray(raw) && raw.length) {
      return raw
        .map((it) => {
          ...
          return { status, createdAt: createdAt || null };
        })
        .filter(Boolean)
    ...
    // fallback: synthesize minimal from known fields
    const items = [];
    const created = parseDate(req?.createdAt);
    if (created) items.push({ status: req?.initialStatus || "pending", createdAt: created });

    const ocularAt = parseDate(req?.scheduledOcularAt) || parseDate(req?.scheduledDate) || parseDate(req?.ocularVisit) || null;
    if (ocularAt) items.push({ status: "ocular_scheduled", createdAt: ocularAt });

    if (req?.acceptedAt) items.push({ status: "accepted", createdAt: parseDate(req.acceptedAt) || null });
    if (req?.declinedAt) items.push({ status: "declined", createdAt: parseDate(req.declinedAt) || null });

    const updated = parseDate(req?.updatedAt);
    if (updated) items.push({ status: req?.status || "pending", createdAt: updated });
  ...
  const findStart = (hist, req) => {
    const fallback = parseDate(req?.createdAt) || null;
    if (!Array.isArray(hist) || !hist.length) return fallback;
    const pending = hist.find((h) => String(h.status || "").toLowerCase().includes("pending"));
    return (pending && parseDate(pending.createdAt)) || parseDate(hist[0].createdAt) || fallback;
  };
  ...
  const getDurationText = (req) => {
    const hist = normalizeHistory(req);
    const start = findStart(hist, req);
    const end = findEnd(hist);
    if (!start) return "N/A";
    const ms = end ? end - start : Date.now() - start;
    const txt = formatDuration(ms);
    return end ? txt : `${txt} (so far)`;
  };
  // ==============================
  ...
  useEffect(() => {
    ...
    setFilteredRequests(filtered);
  }, [searchQuery, statusFilter, priceFilter, requests, addressMap, dateFrom, dateTo, sortOrder]);
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    ...
    try {
      ...
      toast.success(newStatus === "declined" ? "Request declined" : `Request status updated to ${newStatus}`);
  ...
  const handleProposePrice = async (req) => {
    ...
    const canPropose = req.status === "ocular_scheduled" || req.status === "price_declined";
    if (!canPropose) {
      Swal.fire("Not allowed yet", "You can propose a price after an ocular visit or after a client decline.", "info");
      return;
    }
          ...
          {error ? (
            ...
            <TableContainer component={Paper} sx={{ maxHeight: "60vh" }}>
              <Table stickyHeader>
                <TableHead>
                  <TableRow sx={{ bgcolor: "grey.900" }}>
                    {["ID", "Name", "Contact", "Location", "Price", "Status", "Ocular Visit", "Duration", "Actions"].map((head) => (
                      <TableCell
                        key={head}
                        sx={{
                          color: "#202020ff",
                    ...
                    filteredRequests.map((request) => {
                      ...
                      return (
                        <TableRow key={request._id} hover sx={{ cursor: "pointer" }} onClick={() => setSelectedRequest(request)}>
                          ...
                          <TableCell>{request.ocularVisit ? new Date(request.ocularVisit).toLocaleString() : "Not scheduled"}</TableCell>

                          {/* NEW: Duration */}
                          <TableCell>
                            <Typography sx={{ color: "text.secondary" }}>{durationText}</Typography>
                          </TableCell>
