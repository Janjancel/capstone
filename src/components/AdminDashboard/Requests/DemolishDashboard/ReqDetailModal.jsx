...
//     if (s === "scheduled" || s === "ocular_scheduled") return "success";
//     if (s === "declined" || s === "price_declined" || s === "cancelled") return "error";
//     return "warning";
//   };
...
//     docPDF.save(`Demolish_Request_${request._id || "export"}.pdf`);
//   };
...
//     // If server doesn't provide a history array, create minimal timeline:
//     const items = [];
//     // createdAt as initial 'pending' (or initial status)
...
//     // if proposedPrice set, include an event
//     if (request.proposedPrice !== undefined && request.proposedPrice !== null) {
//       const at = parseDate(request.proposedPriceAt) || parseDate(request.updatedAt) || parseDate(request.createdAt);
//       items.push({
//         status: "price_proposed",
//         message: `Proposed price: ₱${Number(request.proposedPrice).toLocaleString()}`,
//         createdAt: at || new Date(),
//       });
//     }

//     // final status (use updatedAt)
//     const updated = parseDate(request.updatedAt) || parseDate(request.scheduledDate) || new Date();
//     items.push({
...
//           const status = it.status || it.name || it.type || (it.to && `changed to ${it.to}`) || "updated";
...
//   // ----------------------------
//   // Terminal statuses we consider for "end" of transaction
...
//     const lowerMatch = hist.find(
//       (h) => h.status && String(h.status).toLowerCase().includes("pending")
//     );
...
//     // find first occurrence of terminal statuses
...
//   const durationMs = transactionStart && transactionEnd ? transactionEnd - transactionStart : transactionStart && !transactionEnd ? Date.now() - transactionStart : null;
...
//                   {transactionEnd ? "End:" : "Current:"}
//                 </Typography>
//                 <Typography sx={{ color: "text.secondary" }}>
//                   {transactionEnd ? formatDateTime(transactionEnd) : "Still in progress"}
//                 </Typography>
//               </Box>
...
const ReqDetailModalDemolish = ({ request = {}, onClose }) => {
  ...
  const transactionStart = findStart(normalizedHistory);
  const transactionEnd = findEnd(normalizedHistory) || null;
  const durationMs =
    transactionStart && transactionEnd ? transactionEnd - transactionStart : transactionStart && !transactionEnd ? Date.now() - transactionStart : null;
  const durationText = durationMs ? formatDuration(durationMs) : "N/A";
  ...
  const invoiceData = useMemo(() => {
    return {
      ...
      items: request.items || [],
      price: request.price ?? request.total ?? request.proposedPrice ?? 0,
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
      margin: 10, // mm margin
      filename: `demolish_request_${invoiceData.id || "invoice"}.pdf`,
        ...
        <Box sx={{ padding: "12mm" }}>
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e0e0e0", pb: 2 }}>
            ...
            <Box sx={{ textAlign: "right" }}>
              <Typography sx={{ fontWeight: 700, fontSize: 18, letterSpacing: "1px" }}>Demolish Request</Typography>
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", mt: 2 }}>
            <Box>
              <Typography sx={{ fontSize: 13 }}>
                <strong>Owner/Seller:</strong> {invoiceData.customer || "N/A"}
              </Typography>
        ...
        <DialogContent dividers>
          ...
          >
            ...
            >
              ...
              {/* Right: Request title + status chip (keeps demolish-specific info) */}
                        ...
                        request.images?.[key] && (
                          <Grid item xs={12} sm={4} key={key}>
                            ...
                            >
                              ...
                              />
                              <Box sx={{ p: 1 }}>
                                <Typography align="center" variant="caption" color="text.secondary" sx={{ textTransform: "capitalize" }}>
                                  {key} view
                                </Typography>
                              </Box>
                            </Card>
