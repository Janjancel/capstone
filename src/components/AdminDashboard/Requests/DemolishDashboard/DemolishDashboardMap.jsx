...
// const DemolishDashboardMap = ({ requests, onClose }) => {
...
// export default DemolishDashboardMap;
...
// ---------- Philippines geofence (approx national bounds with small buffer) ----------
const PH_BOUNDS = L.latLngBounds(
  [4.15, 116.80],  // SW (Mindanao/Palawan vicinity)
  [21.40, 127.20]  // NE (Batanes / Philippine Sea)
);
const PH_CENTER = [12.8797, 121.7740]; // Geographic center of the Philippines

const DemolishDashboardMap = ({ requests, onClose }) => {
  // Guard against undefined/null and keep only numeric coords
  const validLocations = (requests || []).filter(
    (loc) =>
      loc?.location &&
      typeof loc.location.lat === "number" &&
  ...
  // Show only markers that lie inside the Philippines bounds
  const inPhilippines = validLocations.filter((loc) =>
    PH_BOUNDS.contains([loc.location.lat, loc.location.lng])
  );

  // Prefer centering on the first in-bounds point; otherwise fall back to PH center
  ...
  return (
    <div style={{ position: "relative", height: 400, marginBottom: "1rem" }}>
      ...
      <MapContainer
        ...
        maxBounds={PH_BOUNDS}            // lock panning to the Philippines
        maxBoundsViscosity={1.0}         // "sticky" boundary feel
        minZoom={5}
        maxZoom={18}
        worldCopyJump={false}
        scrollWheelZoom
