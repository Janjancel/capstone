...
const ReportDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch orders and sales from Express backend
        ...
        salesData.forEach((sale) => {
          const dateObj = new Date(sale.deliveredAt);
          let key;
          ...
          sale.items?.forEach((item) => {
            itemMap[item.name] =
              (itemMap[item.name] || 0) + item.price * item.quantity;
          });
        });
