...
import DemolishRequestsTab from "./DemolishRequestsTab";
...
export default function MyRequest() {
  ...
  /* ---------------- PRICE HANDLERS (USED BY BOTH TABS) ---------------- */

  const mergeServerUpdateIntoItem = (oldItem, patch, accept, proposed) => {
    const price =
      patch?.agreementPrice ??
      patch?.price ??
      (accept ? proposed : undefined);
    ...
    return {
      ...oldItem,
      ...patch,
      price: accept ? price : oldItem.price,
      proposedPrice: accept ? null : oldItem.proposedPrice,
      status: patch?.status,
      agreementPrice: accept ? price : oldItem.agreementPrice,
    };
  };
  ...
  const handleCounterOffer = async (req) => {
    ...
    const counterPrice = Number(value);

    try {
      setActingId(id);

      const res = await axios.patch(
        `${API_URL}/api/${tab === "sell" ? "sell" : "demolish"}/${id}`,
        {
          proposedPrice: counterPrice,
          status: "awaiting_price_approval",
        }
      ...
      const updater = (list) =>
        list.map((i) =>
          (i._id || i.id) === id
            ? { ...i, ...res.data, proposedPrice: counterPrice }
            : i
        );

      tab === "sell"
        ? setSellRequests(updater)
  ...
  const handleRespondToPrice = async (req, accept) => {
    const id = req._id || req.id;
    const proposed = Number(req.proposedPrice);
    if (!id || !proposed) return;

    const confirm = await Swal.fire({
      title: accept ? "Accept price?" : "Decline price?",
      showCancelButton: true,
    });
    if (!confirm.isConfirmed) return;
    ...
    try {
      ...
      const updater = (list) =>
        list.map((i) =>
          (i._id || i.id) === id
            ? mergeServerUpdateIntoItem(i, res.data, accept, proposed)
            : i
        );
    ...
    <Container sx={{ pt: 14, pb: 6 }}>
      ...
      {loading ? (
        ...
        <SellRequestsTab
          ...
          actingId={actingId}
          handleRespondToPrice={handleRespondToPrice}
